...
export class StudentListComponent implements OnInit {
  ...
  tableColumns: string[] = ['matric_number', 'name', 'course', 'year'];
    ...
    if (!this.formDialogOpened) {
      ...
      dialogRef.afterClosed().subscribe((result)=>{
        ...
        if (result && result.delete) {
          this.confirmDelete(student.matric_number);
        }
        this.reloadData();
      });
    }
  ...
  confirmDelete(matric_number: string) {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {data: `Student ${matric_number}`});
    dialogRef.afterClosed().subscribe(
      (result) => {
        if (result.event == 'yes') {
          this.studentService.deleteStudent(matric_number).subscribe(() => this.reloadData());
        }
      }
    );
  }
